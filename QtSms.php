...
final class QtSms
{
    ...
    public function __construct($user, $pass, $url)
    {
        ...
        $this->on_ssl   = ($urlData['scheme'] !== 'http');
    }
    ...
    private function post_mes($mes, $target, $phl_codename, $sender, $post_id = null, $period = false)
    {
        $in = [
            'action'  => 'post_sms',
            'message' => $mes,
    ...
    private function get_post_request($invars)
    {
        ...
        $send =
            "POST " . $this->path . " HTTP/1.0" . $nn .
            "Host: " . $this->hostname . "" . $nn .
        ...
        if (($fp = @fsockopen($this->getUrl(), $this->getPort(), $errno, $errstr, 30)) !== false) {
            ...
            throw new \RuntimeException('Невозможно соединиться с сервером');
        }
    }
    ...
    {
        return ($this->on_ssl ? 'ssl://' : '') . $this->hostname;
    }
    ...
    private function post_message_phl($mes, $phl_codename, $sender = null, $post_id = null, $period = false)
    {
    ...
    private function get_multipost_request($invars, $file_data)
    {
        ...
        $invars['pass']                 = ($this->pass);
        $invars['CLIENTADR']            = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : false;
        $invars['HTTP_ACCEPT_LANGUAGE'] =
            isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? $_SERVER['HTTP_ACCEPT_LANGUAGE'] : false;
        ...
        if (isset($file_data["name"]) && $file_data["name"] != "") {
            ...
            $fileHeaders .= "Content-Disposition: form-data; name=\"files\"; filename=\"" .
                            $file_data["name"] .
                            "\"" .
                            CRLF;
        ...
        if (($fp = @fsockopen($this->getUrl(), $this->getPort(), $errno, $errstr, 30)) !== false) {
            ...
            if (!$fp2) {
                throw new \RuntimeException("Невозможно открыть файл {$file_data['path']}");
            }
    ...
    private function status_sms_id($sms_id)
    {
    ...
    private function status_sms_group_id($sms_group_id)
    {
        return $this->status_sms(false, false, false, $sms_group_id, false);
    }

    private function status_sms_date($date_from, $date_to, $smstype = 'SENDSMS')
    {
